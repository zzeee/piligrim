...
//$res_tour_ln['title']
            ...
            echo("</tbody></table>");
    ?>
            <div class="panel-heading">Туры по запросу</div>
            <table class="table table-striped" valign="top" border="0">
                <thead>
                <td>Направление</td>
                <td>Минимальая стоимость за человека</td>
                <td>Дней</td>
                </thead>
                <tbody>
                <?php
                $sq='select * from tours where visible=1';
                $rmtl = $mysqli->query($sq);
            while($rmtl_r=$rmtl->fetch_assoc())
            {
                $tt_len=$rmtl_r['blength'];
                $tt_title=$rmtl_r['title'];
                $tt_bprice=$rmtl_r['baseprice'];
                $tt_id=$rmtl_r['id'];
                echo('<tr><td><a href="index.php?action=showatour&tournumber='.$tt_id.'">'.$tt_title.'</a></td><td>'.$tt_bprice.'</td><td>'.$tt_len.'</td></tr>');
        ...
        <div class="jumbotron">
            ...
            <p class="lead">Даты выездов: <? echo (showDatesOsList($id)) ?></p>
        </div>
            ...
            <ul class="list-group">
                ...
                <li class="list-group-item"><div class="row">
                        ...
                        while($rt=$rpic->fetch_assoc()){
                                echo('<a   style="height:300" title="'.$rt['comment'].'" class="col-md-3 col-xs-6 fancyimage thumbnail" data-fancybox-group="group" href="img/' . $rt['name'] . '" ><div class="caption"><p>'.$rt['comment'].'</p></div><img  alt="'.$rt['comment'].'" class="img-responsive"  src="img/' . $rt['name'] . '" vspace=5 class="pull-right" width="300" height="200"/></a>');
                            $count++;
                            if ($count==4) {echo("</div><div class='row'>");$count=0;}
                                //                      echo("</span>");
