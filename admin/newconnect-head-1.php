...
function showTourLine($id, $rl, $dealer)
{
    ...
    $sq="select *, day(date) as day, year(date) as year, month(date) as month, month(now()) as cm from dates where tourid=".$id;
//    echo ($sq);
    ...
    while ($rtl = $rtlist->fetch_assoc()) {
        ...
        $currmonth=$rtl['cm'];
        if ($dealer<>"1")
            $col='<a href="index.php?action=showatour'.$dl.$addline.'&tournumber='.$rtl['tourid'].'">'.$rtl["day"]."</a><br />";
        else
